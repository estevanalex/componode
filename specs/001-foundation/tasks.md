...
> Generated by `/speckit-converge` — gap closure tasks from codebase assessment against spec.md, plan.md, and tasks.md. Constitution Principle VI (Test-First) violations are highest priority.

- [X] T170 [CRITICAL] Replace `crypto.randomUUID()` (UUID v4) with UUID v7 in `packages/backend/src/services/bootstrap-service.ts`, `user-service.ts`, `password-reset-service.ts`, `session-service.ts`, and all other ID-generating code paths per AGENTS.md Entity Identifiers rule (missing)
- [X] T171 [CRITICAL] Implement `packages/backend/src/services/oidc-service.ts` with `initiateLogin` (generate state + PKCE S256, return IdP redirect URL), `handleCallback` (exchange code for tokens, verify ID token, extract oidcSubject + claims, JIT provision or find user, apply role mapping with local override, create session) per FR-009, US3/AC1-5, T128 (missing)
- [X] T172 [P] Add POST `/auth/register` route to `packages/backend/src/routes/auth.ts` — validate with `registerSchema`, rate-limited 3/min per IP, create user with Viewer role, only when `allowSelfRegistration` is enabled per FR-025, US2/AC7-8 (missing)
- [X] T173 Add POST `/auth/oidc/login` and GET `/auth/oidc/callback` routes to `packages/backend/src/routes/auth.ts` per T130, US3/AC1-2 (missing)
- [X] T174 [P] Instrument DB queries in `packages/backend/src/db/connection.ts` — wrap Kysely query execution with `db_query_duration_seconds` histogram (operation label) per FR-016, T158 (missing)
- [X] T175 [P] Add DB query child spans to `packages/backend/src/plugins/tracing.ts` — child span per DB query under the request root span per T159 (missing)
- [X] T176 [P] Write `packages/backend/test/integration/auth.test.ts` — login flow, session check, logout per T092-T094 (missing)
- [X] T177 [P] Write `packages/backend/test/integration/health.test.ts` — GET /api/v1/health per T095 (missing)
- [X] T178 [P] Write `packages/backend/test/integration/users.test.ts` — user CRUD, duplicate username per T114 (missing)
- [X] T179 [P] Write `packages/backend/test/integration/rbac.test.ts` — Viewer/Editor/Admin enforcement per T115 (missing)
- [X] T180 [P] Write `packages/backend/test/integration/oidc.test.ts` — OIDC flow + error cases per T125-T126 (missing)
- [X] T181 [P] Write `packages/backend/test/integration/sessions.test.ts` — session persistence, revoke, admin list per T139 (missing)
- [X] T182 [P] Write `packages/backend/test/integration/rate-limit.test.ts` — 6th login → 429, 4th register → 429 per T140 (missing)
- [X] T183 [P] Write `packages/backend/test/integration/csrf.test.ts` — POST without token → 403, matching → success per T141 (missing)
- [X] T184 [P] Write `packages/backend/test/integration/password.test.ts` — password change + reset flow per T142-T143 (missing)
- [X] T185 [P] Write `packages/backend/test/integration/metrics.test.ts` — GET /metrics → 5+ metric families per T153 (missing)
- [X] T186 [P] Write `packages/backend/test/unit/logging.test.ts` — Pino redaction filter verification per T154 (missing)
- [X] T187 [P] Write `packages/backend/test/unit/password-reset.test.ts` — token generation, confirm, reuse, expiry per T143 (missing)
- [X] T188 [P] Add shadcn/ui Button component to `packages/frontend/src/components/ui/button.tsx` per T102 (missing)
- [X] T189 [P] Add shadcn/ui Input component to `packages/frontend/src/components/ui/input.tsx` per T103 (missing)
- [X] T190 [P] Add shadcn/ui Label component to `packages/frontend/src/components/ui/label.tsx` per T104 (missing)
- [X] T191 [P] Add shadcn/ui Card component to `packages/frontend/src/components/ui/card.tsx` per T105 (missing)
- [X] T192 [P] Add shadcn/ui Table component to `packages/frontend/src/components/ui/table.tsx` per T119 (missing)
- [X] T193 [P] Add shadcn/ui Dialog component to `packages/frontend/src/components/ui/dialog.tsx` per T120 (missing)
- [X] T194 [P] Add shadcn/ui Select component to `packages/frontend/src/components/ui/select.tsx` per T121 (missing)
- [X] T195 [P] Add shadcn/ui Form component to `packages/frontend/src/components/ui/form.tsx` per T132 (missing)
- [X] T196 [P] Add shadcn/ui Switch component to `packages/frontend/src/components/ui/switch.tsx` per T133 (missing)
- [X] T197 [P] Add shadcn/ui Badge component to `packages/frontend/src/components/ui/badge.tsx` per T148 (missing)
- [X] T198 [P] Add shadcn/ui Toaster (Sonner) component to `packages/frontend/src/components/ui/sonner.tsx` per T162 (missing)
- [X] T199 Create `packages/frontend/src/components/layout/nav.tsx` — nav layout with links to dashboard sections, user info, logout button per T111 (missing)
- [X] T200 [P] Create `packages/frontend/src/api/hooks/auth.ts` — useLogin, useLogout, useSession TanStack Query hooks per T112 (missing)
- [X] T201 [P] Create `packages/frontend/src/api/hooks/users.ts` — useUsers, useCreateUser, useUpdateUser per T123 (missing)
- [X] T202 [P] Create `packages/frontend/src/api/hooks/settings.ts` — useSettings, useUpdateSettings, useOidcConfig, useUpdateOidcConfig per T137 (missing)
- [X] T203 [P] Create `packages/frontend/src/api/hooks/sessions.ts` — useSessions, useRevokeSession per T151 (missing)
- [X] T204 [P] Create `packages/frontend/src/api/hooks/password.ts` — useChangePassword, useResetPassword per T151 (missing)
- [X] T205 Create `packages/frontend/src/pages/register.tsx` — self-registration page with username/password form, submit to POST /api/v1/auth/register, redirect to /login on success, error display for 429/403, WCAG 2.1 AA per FR-025, T135 (missing)
- [X] T206 Create `packages/frontend/src/pages/oidc-callback.tsx` — handles redirect from IdP, shows loading state, redirects to dashboard on success per T136 (missing)
- [X] T207 Add "Login with OIDC" button to `packages/frontend/src/pages/login.tsx` — conditionally rendered if OIDC is enabled (check via public endpoint) per T135, US3/AC1 (missing)
- [X] T208 Expand `packages/frontend/src/pages/settings.tsx` — add editable app settings form (PATCH /settings), OIDC config section (PUT /settings/oidc with issuer, clientId, clientSecretRef, roleClaimPath, claimValueField, roleMapping), password change form (POST /auth/password/change) per T134, T150 (partial)
- [X] T209 Add error boundary to `packages/frontend/src/app.tsx` — catches render errors, shows error message with retry button per T168 (missing)
- [X] T210 Add `/register` and `/auth/oidc/callback` routes to `packages/frontend/src/routes.tsx` per T080, T113 (missing)
- [X] T211 [P] Write `packages/frontend/test/unit/external-link.test.ts` — verify ExternalLink renders with correct rel/target attributes per T124 (missing)
- [X] T212 Add OIDC issuer discovery validation to `packages/backend/src/services/settings-service.ts` `updateOidcConfig` — test issuer reachability before saving per T127 (partial)
- [X] T213 Verify migration creates all 28 tables per spec.md FR-001/SC-002 — current migration creates 24 tables, verify against `data-model.md` and add any missing tables per FR-001 (partial)
